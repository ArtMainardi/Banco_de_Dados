...
FROM
    ...
    JOIN vendas AS v ON v.id_funcionario = f.id_funcionario 
GROUP BY 
    f.nome_funcionario, 
    c.cargo, 
    t.turno;
...
# Query 3
select id_cliente, idade_cliente, pontos_cliente from clientes where pontos_cliente > 500 and idade_cliente < 40;
...
# == BARBARA == #
# Query 6
SELECT c.nome_categoria AS Categoria, MAX(p.preco_produto) AS Preco_Maximo, MIN(p.preco_produto) AS Preco_Minimo
FROM produtos p
